...
- Homepage photo carousel using five committed placeholder images near the top of the center column so the public slideshow treatment can be reviewed before gallery-backed wiring lands
- Homepage photo carousel now keeps a fixed widescreen frame, uses contained placeholder-image scaling so venue photos stay fully visible without crop, and switches to a separate caption panel on smaller screens so the mobile image area remains usable while the gallery-backed version is still pending
