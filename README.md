...
- The Event Editor loads real event definitions from that same catalog in pages of 10, lets EventManager users save drafts, publish or archive existing events, delete events through a warning-first confirm step without discarding unsaved edits to a different event, keeps recurrence defaults aligned with the selected event date unless staff intentionally override them, rejects stale saves after another session deletes an event, normalizes relative image paths for admin previews, and locks event actions during in-flight saves or deletes, with a visible Saving state while a save is in progress
