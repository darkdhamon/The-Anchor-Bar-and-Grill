...
- Homepage photo carousel now keeps a fixed widescreen frame, uses contained placeholder-image scaling so venue photos stay fully visible without crop, lets the welcome headline and lead start the section before the follow-on copy wraps around a half-width carousel on larger screens, stacks the carousel first ahead of the content once the screen narrows, auto-rotates every ten seconds, and pauses while hovered or touched
