...
- The public menu now uses a left sidebar for service selection, keeps weekly hours focused on the active menu only, defaults to the service that is open now or the next one to open when guests arrive without a `tab` query string, and opens menu categories as accordion sections with the first category expanded by default
