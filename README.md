...
- The Drinks workspace now lets staff create new drink sections without fighting the menu-visibility chips, so a new drink section can be saved once `Drinks` is enabled instead of failing with a false “Drink sections must appear on Drinks” validation error
