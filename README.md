...
- The Event Editor loads real event definitions from that same catalog in stable pages of 10, keeps reusable badge suggestions available across the full catalog, returns to the page containing a saved event, and lets EventManager users save drafts, publish or archive existing events. It deletes events through a warning-first confirm step without discarding unsaved edits to a different event, keeps one-time recurrence defaults aligned with the selected event date without changing an existing recurring cadence, rejects contradictory next-day schedules and stale saves, supports reverting an off-page selection, normalizes relative image paths for admin previews, and serializes page/save/delete actions to prevent races. Successful saves, publications, archives, and deletions are stored in the database and shown in the protected Recent event activity panel; `logs/event-operations.log` is used when database logging fails.
