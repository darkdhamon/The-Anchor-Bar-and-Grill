...
- The Menu Editor now supports direct menu-item image uploads, saving processed `.webp` files under `wwwroot/images/gallery/menuitems`, rejecting raw uploads over `50 MB`, compressing saved uploads to `5 MB` or smaller, handling common phone-camera JPG uploads through server-side normalization, and keeping the manual image-path workflow behind an `Advanced image path` reveal
