...
- The Menu Editor now separates `Food`, `Drinks`, and `Hours` into a tabbed admin console, with archive-state browsing, greyed archived records, contextual create actions, subsection headers inside parent section cards for ordering, a sticky focused detail panel, and a built-in right-side quick guide for staff
