...
- Database-backed Event Editor with save-draft, publish, archive, and delete workflows, plus local-time scheduling, recurring-event controls, optional images, and reusable promo badges
...
- The Event Editor now loads real event definitions from that same catalog, lets EventManager users save drafts, publish or archive existing events, delete events with confirmation, and keep recurring scheduling aligned with the one-year recurrence cap
