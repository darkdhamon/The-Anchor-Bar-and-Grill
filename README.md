...
- Homepage photo carousel now keeps a fixed widescreen frame, uses contained placeholder-image scaling so venue photos stay fully visible without crop, starts with captions hidden behind a simple show/hide link, sits higher beside the welcome copy on larger screens, stacks the carousel first ahead of the content once the screen narrows, auto-rotates every ten seconds, and pauses while hovered or touched
