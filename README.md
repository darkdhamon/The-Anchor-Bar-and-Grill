...
- Database-backed Event Editor with save-draft, publish, archive, and delete workflows, plus local-time scheduling, recurring-event controls that stay aligned with the selected new-event date, optional images, normalized relative-path image previews, reusable promo badges, a warning-first confirm-delete flow, and in-flight action locking to prevent save/delete races
...
- The Event Editor now loads real event definitions from that same catalog, lets EventManager users save drafts, publish or archive existing events, delete events through a warning-first confirm step, keeps new-event recurrence defaults aligned with the selected event date unless staff intentionally override them, normalizes relative image paths for admin previews, and locks event actions during in-flight saves or deletes, with a visible Saving state while a save is in progress
