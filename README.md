...
- Database-backed Event Editor with save-draft, publish, archive, and delete workflows, plus local-time scheduling, recurring-event controls that stay aligned with the selected event date, optional images, normalized relative-path image previews, reusable promo badges, paged catalog browsing, stale-update protection, preservation of unrelated unsaved edits during deletion, a warning-first confirm-delete flow, and in-flight action locking to prevent save/delete races
...
- The Event Editor loads real event definitions from that same catalog in stable pages of 10, keeps reusable badge suggestions available across the full catalog, returns to the page containing a saved event, and lets EventManager users save drafts, publish or archive existing events. It deletes events through a warning-first confirm step without discarding unsaved edits to a different event, keeps one-time recurrence defaults aligned with the selected event date without changing an existing recurring cadence, rejects contradictory next-day schedules and stale concurrent saves or deletions, supports reverting an off-page selection, normalizes relative image paths for admin previews, and serializes page/save/delete/new-record actions to prevent races. Successful saves and explicit draft, publish, archive, and delete actions are labeled accurately in the audit history. Records are stored in the database and shown to EventManager users in the editor and Admin users at `/admin/event-activity`; `logs/event-operations.log` is used when database logging fails and its outage records are included in those activity views after recovery.
